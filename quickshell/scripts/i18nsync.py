...
TRANSLATIONS_DIR = SHELL_DIR / "translations"
EN_JSON = TRANSLATIONS_DIR / "en.json"
TEMPLATE_JSON = TRANSLATIONS_DIR / "template.json"
POEXPORTS_DIR = TRANSLATIONS_DIR / "poexports"
EXTRACT_SCRIPT = TRANSLATIONS_DIR / "extract_translations.py"
SYNC_STATE = REPO_ROOT / ".git" / "i18n_sync_state.json"

# git-relative path for staging/diffing the translations directory.
TRANSLATIONS_REL = TRANSLATIONS_DIR.relative_to(REPO_ROOT).as_posix()
EN_JSON_REL = EN_JSON.relative_to(REPO_ROOT).as_posix()

API_TOKEN_ENV = "DCAL_POEDITOR_API_TOKEN"
PROJECT_ID_ENV = "DCAL_POEDITOR_PROJECT_ID"
...
def download_translations(api_token, project_id):
    if not LANGUAGES:
        info("No languages configured yet - skipping download")
        return False
...
def check_sync_status():
    api_token = get_env_or_error(API_TOKEN_ENV)
    project_id = get_env_or_error(PROJECT_ID_ENV)
    ...
    first_file = POEXPORTS_DIR / LANGUAGES[first_lang]
    export_resp = poeditor_request('projects/export', {
        'api_token': api_token,
        'id': project_id,
        'language': first_lang,
...
def run_test():
    ...
    terms_with_context = sum(1 for entry in current_en if entry.get('comment'))
    if terms_with_context:
        success(f"✓ Found {terms_with_context} terms with translator contexts")
...
def run_check():
    try:
        if check_sync_status():
            error("i18n out of sync - run 'make i18n-sync' first")
...
def run_sync():
    ...
    if strings_changed or translations_changed:
        subprocess.run(['git', 'add', TRANSLATIONS_REL], cwd=REPO_ROOT)
        save_sync_state()
...
def run_local():
    ...
    added = set(new_terms) - set(old_terms)
...
def main():
    if len(sys.argv) < 2:
        error("Usage: i18nsync.py [check|sync|push|test|local]")
